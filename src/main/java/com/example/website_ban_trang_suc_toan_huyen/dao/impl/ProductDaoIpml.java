...
public class ProductDaoIpml implements ProductDao {
    ...
    @Override
    public Long countV2(Integer pageIndex, Integer pageSize, String keyword, ProductEntity.StatusEnum status, List<UUID> materialId, UUID vendorId, UUID categoryId, UUID accessoryId, BigDecimal startPrice, BigDecimal endPrice, String sortBy, ProductEntity.ProductGender gender) {
        Map<String, Object> values = new HashMap<>();
        StringBuilder sql = new StringBuilder("SELECT COUNT(p) FROM ProductEntity p ");
        sql.append(createWhereQueryV2(values,keyword,status,materialId,vendorId,categoryId,accessoryId,startPrice,endPrice,gender));
        Query query = entityManager.createQuery(sql.toString(), Long.class);
